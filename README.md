...
- 주제 폴더: `topic01_setup` ~ `topic07_reference`
- 실습 폴더(기존 `00~10`)는 주제별 `practice/` 하위로 통합됨
...
- **환경 구축(WSL/VirtualBox/K3s/SSH/kubeconfig)**: `topic01_setup/`
- **코어 워크로드(Pod/Label/ReplicaSet/Deployment)**: `topic02_core_workloads/`
- **대시보드/관측(Ingress/Metrics/Dashboard/재접속)**: `topic03_dashboard_observability/`
- **스케일링/안정성(HPA/Fortio/Manifest/Probe/YAML)**: `topic04_scaling_reliability/`
- **네트워크/Ingress(Service/ClusterIP/라우팅 디버깅)**: `topic05_network_ingress/`
- **종합 트러블슈팅**: `topic06_troubleshooting/`
- **치트시트/용어/샌드박스**: `topic07_reference/`

## 이미지 자료

실습 스크린샷은 각 `topicXX` 폴더 내에서 해당 MD와 함께 관리됩니다.

## 활용 방법

1. `topic01_setup`으로 기본 실습 환경을 구성합니다.
2. `topic02_core_workloads`로 Pod/ReplicaSet/Deployment 흐름을 익힙니다.
3. `topic03_dashboard_observability`와 `topic05_network_ingress`로 운영/라우팅을 점검합니다.
4. `topic04_scaling_reliability`로 HPA와 Probe/Manifest 실습을 진행합니다.
5. `topic06_troubleshooting`과 `topic07_reference`로 문제 해결/복습을 마무리합니다.
